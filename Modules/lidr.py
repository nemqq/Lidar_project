...
def average_r(tab):
    ...
    for i in difference:
        calc_sum += i
    print(difference)
    result = (calc_sum/len(difference))
...
def Detect_OBST(tab_z, tab_b, r_p):
    ...
    for i in tab_avg:
        if r_p[k]:
            if i > 134:
                print('obst nr.: ' + str(k+1) + ' is low')
                object['high'] = 0
            elif 134 > i > 60:
                print('obst nr.: '+str(k+1)+' is high ')
                object['high'] = 1
            else:
    ...
    for k in range(len(obst_size)):
        if obst_size[k] != 0:
            if 0 < obst_size[k] <= 2:
                print("obstacle nr.:" + str(k+1) + " is very small")    # 3,3cm, possible a disturbance
                (obstacle[k])['width'] = 0
            elif 2 < obst_size[k] <= 4:
                print("obstacle nr.:" + str(k+1) + " is small")         # estimated 10cm
                (obstacle[k])['width'] = 1
            elif 4 < obst_size[k] < 10:
                print("obstacle nr.:" + str(k+1) + " is average")       # estimated od 10cm do 30cm
                (obstacle[k])['width'] = 2
            elif obst_size[k] > 10:
                print("obstacle nr.:" + str(k+1) + " is huge")          # above 1cm
                (obstacle[k])['width'] = 3
...
def pre_detection(tab, ref_tab, u_scope=7.5):
    ...
    :param u_scope: from which an obstacle is detected
